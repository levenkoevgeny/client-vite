  ...
  >
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                      type="text"
                      class="form-control"
                      v-model="selectedItem.health_group"
                      required
                  />
                </div>
              </div>
    ...
    <template v-slot:search-form>
      <div class="row">
        <div class="col-12">
          <div class="mb-3">
            <label for="health_group__icontains" class="form-label">Группа предназнечения</label>
            <input
                type="text"
                class="form-control"
                id="health_group__icontains"
                v-model="searchForm.health_group__icontains"
            />
          </div>
        </div>
      </div>
...
export default {
  name: "HealthGroupListView",
  components: { BaseListLayout },
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getHealthGroupAPIInstance(),
      itemForm: Object.assign({}, getHealthGroupAPIInstance().formData),
      searchForm: Object.assign({}, getHealthGroupAPIInstance().searchObj),
      selectedItem: Object.assign({}, getHealthGroupAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("healthGroup/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("healthGroup/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch("healthGroup/actionDeleteItem", this.deleteItemId)
      } catch (error) {
      } finally {
    ...
    async deleteCheckedItemsHandler() {
      this.mainItemList.results.map(async (item) => {
        if (item.isSelected) {
          await this.$store.dispatch("healthGroup/actionDeleteItem", item.id)
        }
      })
    ...
    debouncedSearch: debounce(async function () {
      try {
        await this.$store.dispatch("healthGroup/actionGetList", this.searchForm)
      } catch (e) {
        this.isError = true
      } finally {
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "healthGroup/getList",
    }),
  },
  watch: {
    searchForm: {
