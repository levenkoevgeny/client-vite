<template>
  ...
  >
          ...
          <div class="modal-content">
            ...
            <form @submit.prevent="addNewItem">
              <div class="modal-body">
                <div class="mb-3">
                  <label
                    for="id_entrance_category_name_create"
                    class="form-label"
                    >Категория поступающих</label
                  >
                  <input
                    id="id_entrance_category_name_create"
                    type="text"
                    class="form-control"
                    v-model="itemForm.entrance_category_name"
                    required
                  />
                </div>
              </div>
          ...
          <div class="modal-content">
            ...
            <form @submit.prevent="updateMainItemInList">
              <div class="modal-body">
                <div class="mb-3">
                  <label
                    for="id_entrance_category_name_update"
                    class="form-label"
                    >Категория поступающих</label
                  >
                  <input
                    id="id_entrance_category_name_update"
                    type="text"
                    class="form-control"
                    v-model="selectedItem.entrance_category_name"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      <tr
        v-for="entranceCategory in orderedMainList"
        :key="entranceCategory.id"
        @dblclick.stop="showUpdateMainItemModalInList(entranceCategory.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
          >
            <input
              type="checkbox"
              class="form-check-input my-0"
              v-model="entranceCategory.isSelected"
            />
          </div>
        </td>
        <td>{{ entranceCategory.entrance_category_name }}</td>
        <td>
          {{
            new Date(entranceCategory.date_time_created).toLocaleString(
              "ru-RU",
              {
                day: "numeric",
                month: "long",
        ...
        <td>
          {{
            new Date(entranceCategory.date_time_updated).toLocaleString(
              "ru-RU",
              {
                day: "numeric",
                month: "long",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(entranceCategory.id)"
    ...
    <template v-slot:search-form>
      <div class="row">
        <div class="col-12">
          <div class="mb-3">
            <label for="entrance_category_name__icontains" class="form-label"
              >Категория поступающих</label
            >
            <input
              type="text"
              class="form-control"
              id="entrance_category_name__icontains"
              v-model="searchForm.entrance_category_name__icontains"
            />
          </div>
        </div>
      </div>
...
<script>
import getEntranceCategoryAPIInstance from "@/api/cadet/entranceCategoryAPI.js"
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getEntranceCategoryAPIInstance(),
      itemForm: Object.assign({}, getEntranceCategoryAPIInstance().formData),
      searchForm: Object.assign({}, getEntranceCategoryAPIInstance().searchObj),
      selectedItem: Object.assign(
        {},
        getEntranceCategoryAPIInstance().formData,
      ),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    debouncedSearch: debounce(async function () {
      try {
        await this.$store.dispatch(
          "entranceCategory/actionGetList",
          this.searchForm,
        )
      } catch (e) {
        this.isError = true
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("entranceCategory/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("entranceCategory/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "entranceCategory/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "entranceCategory/getList",
    }),
  },
  watch: {
    searchForm: {
