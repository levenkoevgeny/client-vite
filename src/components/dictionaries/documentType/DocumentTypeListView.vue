<template>
  ...
  >
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    class="form-control"
                    v-model="itemForm.document_type"
                    required
                  />
                </div>
              </div>
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    class="form-control"
                    v-model="selectedItem.document_type"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      <tr
        v-for="doc_type in orderedMainList"
        :key="doc_type.id"
        @dblclick.stop="showUpdateMainItemModalInList(doc_type.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
        ...
        <td>
          {{
            new Date(doc_type.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(doc_type.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(doc_type.id)"
    ...
    <template v-slot:search-form>
      <div class="row">
        <div class="col-12">
          <div class="mb-3">
            <label for="document_type__icontains" class="form-label"
              >Направление ОРД</label
            >
            <input
              type="text"
              class="form-control"
              id="document_type__icontains"
              v-model="searchForm.document_type__icontains"
            />
          </div>
        </div>
      </div>
...
<script>
import getDocumentTypeAPIInstance from "@/api/cadet/documentTypeAPI.js"
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getDocumentTypeAPIInstance(),
      itemForm: Object.assign({}, getDocumentTypeAPIInstance().formData),
      searchForm: Object.assign({}, getDocumentTypeAPIInstance().searchObj),
      selectedItem: Object.assign({}, getDocumentTypeAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("documentType/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("documentType/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteCheckedItemsHandler() {
      this.mainItemList.results.map(async (item) => {
        if (item.isSelected) {
          await this.$store.dispatch("documentType/actionDeleteItem", item.id)
        }
      })
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "documentType/getList",
    }),
  },
  watch: {
    searchForm: {
