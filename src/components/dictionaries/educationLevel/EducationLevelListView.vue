<template>
  ...
  >
          ...
          <div class="modal-content">
            ...
            <form @submit.prevent="addNewItem">
              <div class="modal-body">
                <div class="mb-3">
                  <label for="id_education_level_create" class="form-label"
                    >Уровень образования</label
                  >
                  <input
                    id="id_education_level_create"
                    type="text"
                    class="form-control"
                    v-model="itemForm.education_level"
                    required
                  />
                </div>
              </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <form @submit.prevent="updateMainItemInList">
              <div class="modal-body">
                <div class="mb-3">
                  <label for="id_education_level_update" class="form-label"
                    >Уровень образования</label
                  >
                  <input
                    id="id_education_level_update"
                    type="text"
                    class="form-control"
                    v-model="selectedItem.education_level"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      <tr
        v-for="educationLevel in orderedMainList"
        :key="educationLevel.id"
        @dblclick.stop="showUpdateMainItemModalInList(educationLevel.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
          >
            <input
              type="checkbox"
              class="form-check-input my-0"
              v-model="educationLevel.isSelected"
            />
          </div>
        </td>
        <td>{{ educationLevel.education_level }}</td>
        <td>
          {{
            new Date(educationLevel.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(educationLevel.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(educationLevel.id)"
        ...
        <div class="col-12">
          <div class="mb-3">
            ...
            <input
              ...
              v-model="searchForm.education_level__icontains"
            />
          </div>
        </div>
      </div>
...
<script>
import getEducationLevelAPIInstance from "@/api/cadet/educationLevelAPI.js"
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getEducationLevelAPIInstance(),
      itemForm: Object.assign({}, getEducationLevelAPIInstance().formData),
      searchForm: Object.assign({}, getEducationLevelAPIInstance().searchObj),
      selectedItem: Object.assign({}, getEducationLevelAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("educationLevel/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("educationLevel/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "educationLevel/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
    ...
    async deleteCheckedItemsHandler() {
      this.mainItemList.results.map(async (item) => {
        if (item.isSelected) {
          await this.$store.dispatch("educationLevel/actionDeleteItem", item.id)
        }
      })
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "educationLevel/getList",
    }),
  },
  watch: {
    searchForm: {
