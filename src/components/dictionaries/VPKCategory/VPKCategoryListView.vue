<template>
  ...
  >
          ...
          <div class="modal-content">
            ...
            <form @submit.prevent="addNewItem">
              <div class="modal-body">
                <div class="mb-3">
                  <label for="id_category_create" class="form-label"
                    >Категория</label
                  >
                  <input
                    id="id_category_create"
                    type="text"
                    class="form-control"
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <form @submit.prevent="updateMainItemInList">
              <div class="modal-body">
                <div class="mb-3">
                  <label for="id_category_update" class="form-label"
                    >Категория</label
                  >
                  <input
    ...
    <template v-slot:tbody>
      <tr
        v-for="vpk_category in orderedMainList"
        :key="vpk_category.id"
        @dblclick.stop="showUpdateMainItemModalInList(vpk_category.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
          >
            <input
              type="checkbox"
              class="form-check-input my-0"
              v-model="vpk_category.isSelected"
            />
          </div>
        </td>
        <td>{{ vpk_category.category }}</td>
        <td>
          {{
            new Date(vpk_category.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(vpk_category.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(vpk_category.id)"
...
<script>
import getVpkCategoriesAPIInstance from "@/api/cadet/vpkCategoryAPI.js"
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getVpkCategoriesAPIInstance(),
      itemForm: Object.assign({}, getVpkCategoriesAPIInstance().formData),
      searchForm: Object.assign({}, getVpkCategoriesAPIInstance().searchObj),
      selectedItem: Object.assign({}, getVpkCategoriesAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("vpkCategories/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("vpkCategories/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "vpkCategories/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
    ...
    async deleteCheckedItemsHandler() {
      this.mainItemList.results.map(async (item) => {
        if (item.isSelected) {
          await this.$store.dispatch("vpkCategories/actionDeleteItem", item.id)
        }
      })
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "vpkCategories/getList",
    }),
  },
  watch: {
    searchForm: {
