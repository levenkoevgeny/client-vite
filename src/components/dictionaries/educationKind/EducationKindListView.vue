<template>
  <base-list-layout
    ...
    title="Виды учреждений образования"
  >
          ...
          <div class="modal-content">
            ...
            <form @submit.prevent="addNewItem">
              <div class="modal-body">
                <div class="mb-3">
                  <label for="id_education_create" class="form-label"
                    >Вид учреждения образования</label
                  >
                  <input
                    id="id_education_create"
                    type="text"
                    class="form-control"
        ...
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <form @submit.prevent="updateMainItemInList">
              <div class="modal-body">
                <div class="mb-3">
                  <label for="id_education_update" class="form-label"
                    >Вид учреждения образования</label
                  >
                  <input
                    id="id_education_update"
                    type="text"
                    class="form-control"
                    v-model="selectedItem.education"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      <tr
        v-for="educationKind in orderedMainList"
        :key="educationKind.id"
        @dblclick.stop="showUpdateMainItemModalInList(educationKind.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
          >
            <input
              type="checkbox"
              class="form-check-input my-0"
              v-model="educationKind.isSelected"
            />
          </div>
        </td>
        <td>{{ educationKind.education }}</td>
        <td>
          {{
            new Date(educationKind.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(educationKind.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(educationKind.id)"
        ...
        <div class="col-12">
          <div class="mb-3">
            ...
            <input
              ...
              v-model="searchForm.education__icontains"
            />
          </div>
        </div>
      </div>
...
<script>
import getEducationKindAPIInstance from "@/api/cadet/educationKindAPI.js"
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getEducationKindAPIInstance(),
      itemForm: Object.assign({}, getEducationKindAPIInstance().formData),
      searchForm: Object.assign({}, getEducationKindAPIInstance().searchObj),
      selectedItem: Object.assign({}, getEducationKindAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("educationKind/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("educationKind/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "educationKind/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
    ...
    async deleteCheckedItemsHandler() {
      this.mainItemList.results.map(async (item) => {
        if (item.isSelected) {
          await this.$store.dispatch("educationKind/actionDeleteItem", item.id)
        }
      })
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "educationKind/getList",
    }),
  },
  watch: {
    searchForm: {
